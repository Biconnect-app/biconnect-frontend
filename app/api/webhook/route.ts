...
export async function POST(request: NextRequest) {
  try {
    ...
    const quantity = parseFloat(trade_data.quantity);
    ...
    if (action === 'buy' || action === 'sell') {
      ...
      try {
        ...
        logger.error(
          ...
          '[WEBHOOK] ❌ Error ejecutando orden SPOT'
        );
        const errorMessage =
          error?.response?.data?.msg ||
          error?.message ||
          'Error desconocido al crear orden SPOT';
    ...
    // ──────────────────────────────────────────────
      ...
      try {
        ...
        const response = await futuresClient.newOrder(
          symbol,
          trade_data.side!,
          'MARKET',
          orderParams
        ...
        logger.error(
          { error, originalError: error },
          '[WEBHOOK] ❌ Error ejecutando orden FUTURES'
        );
        const errorMessage =
          error?.response?.data?.msg ||
          error?.message ||
          'Error desconocido al crear orden FUTURES';
    ...
    logger.error(
      { error, originalError: error },
      '[WEBHOOK] ❌ Error en la ejecución del webhook'
    );
