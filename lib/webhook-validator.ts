...
export class WebhookValidator {
  ...
  async validate(data: WebhookData): Promise<ValidationResult> {
    ...
    const percentage = parseFloat(String(data.percentage || 0));
    const usdt_amount = parseFloat(String(data.usdt_amount || 0));
    const optionalValid =
      'quantity' in data || 'qty' in data || percentage > 0 || usdt_amount > 0;
    ...
    try {
      ...
      leverage =
        action === 'long' || action === 'short'
          ? parseInt(String(data.leverage || 1))
          : undefined;
      price = 'price' in data ? parseFloat(String(data.price)) : undefined;
      close_position = Boolean(data.close_position || false);
      quantity =
        'quantity' in data || 'qty' in data
          ? parseFloat(String(data.quantity || data.qty))
          : undefined;
      ...
      if (missing.length > 0 || !optionalValid || invalidAction) {
        ...
        if (invalidAction) {
          msgLines.push(
            `❌ Acción inválida: '${
              data.action
            }'. Las acciones permitidas son: ${Array.from(validActions).join(
              ', '
            )}\n`
          );
        }
...
⚠️ Al menos uno de los campos opcionales 'quantity', 'percentage' o 'usdt_amount' debe estar presente.`
      ...
      logger.error(
        { error: msg, originalError: error },
        '[VALIDATOR] Error de tipo en campos'
      );
      return { valid: false, error: msg, log_summary: msg };
    }
    ...
    try {
      ...
      logger.error(
        { error: msg, originalError: error },
        '[VALIDATOR] Error obteniendo info'
      );
      return { valid: false, error: msg, log_summary: msg };
    }
    ...
    if (percentage > 0 || usdt_amount > 0) {
      try {
        quantity = await this._calculateQuantity(
          metadata,
          percentage,
          usdt_amount,
          priceResult,
        ...
        logger.error(
          { error: msg, originalError: error },
          '[VALIDATOR] Error calculando quantity'
        );
        return { valid: false, error: msg, log_summary: msg };
      }
    }
    ...
    logger.info(
      ...
      '[VALIDATOR] 🧪 Acción validada con cantidad'
    );

    try {
      if (action === 'buy' || action === 'sell') {
        return await this._validateSpot(
          symbol,
          action,
          quantity,
          priceResult,
      ...
      logger.error(
        { error: msg, originalError: error, action },
        '[VALIDATOR] Error en validación'
      );
      return { valid: false, error: msg, log_summary: msg };
    }
  ...
  private async _getPrice(symbol: string, action: string): Promise<number> {
    try {
      ...
      const price = parseFloat(ticker.data.price || ticker.data);
    ...
    try {
      ...
      if (action === 'buy' || action === 'sell') {
        ...
        try {
          ...
          if (Array.isArray(accountData.balances)) {
            for (const b of accountData.balances) {
              balances[b.asset] = parseFloat(b.free || 0);
            }
          }
        } catch (error: any) {
          throw new Error(
            `❌ Error al obtener información de cuenta SPOT: ${error.message}`
          );
        }
        ...
        try {
          ...
          throw new Error(
            `❌ Símbolo '${symbol}' no existe o no se pudo obtener info en SPOT`
          );
        }

        // Validar fondos disponibles
        const assetRequired =
          action === 'buy' ? symbolInfo.quoteAsset : symbolInfo.baseAsset;
        ...
        try {
          ...
          throw new Error(
            `❌ Error al obtener info de cuenta o símbolo FUTURES: ${error.message}`
          );
        }
        ...
        if (close_position) {
          ...
          try {
            ...
            const positionsData = Array.isArray(positions.data)
              ? positions.data
              : [];
            const baseAsset = symbolInfo.baseAsset;
            const positionData = positionsData.find(
              (p: any) => p.symbol === symbol && p.positionSide === 'BOTH'
            );
            if (!positionData) {
              throw new Error(
                `❌ No se encontró posición abierta en ${symbol}`
              );
            }
            const positionAmt = parseFloat(positionData.positionAmt || 0);
            if (positionAmt === 0) {
              throw new Error(
                `❌ No hay posición activa para cerrar en ${symbol}`
              );
            }
            balances = { [baseAsset]: Math.abs(positionAmt) };
          } catch (error: any) {
            throw new Error(
              `❌ Error al obtener posición abierta para cierre: ${error.message}`
            );
          }
        } else {
          ...
          if (Array.isArray(accountData.assets)) {
            for (const a of accountData.assets) {
              balances[a.asset] = parseFloat(a.availableBalance || 0);
            }
          }
        ...
        throw new Error(
          `❌ Acción '${action}' no reconocida para obtener info`
        );
      }
  ...
  private async _calculateQuantity(
    ...
    if (action === 'buy' || action === 'sell') {
      ...
      if (action === 'buy') {
        const available = balances[quoteAsset] || 0;
        const amountToUse =
          percentage > 0 ? (available * percentage) / 100 : usdt_amount;
        ...
        if (percentage > 0) {
          ...
          throw new Error(
            'Debe especificarse porcentaje o usdt_amount para vender.'
          );
        }
      }
    } else if (action === 'long' || action === 'short') {
      if (close_position) {
        ...
        if (percentage > 0) {
          ...
          throw new Error(
            'Debe especificarse porcentaje o usdt_amount para cerrar posición.'
          );
        }
      } else {
  ...
  private async _validateSpot(
    ...
    try {
      ...
      const minNotional =
        parseFloat(filters['MIN_NOTIONAL']?.minNotional || '0') ||
        parseFloat(filters['NOTIONAL']?.minNotional || '0');

      logger.info(
        { minQty, stepSize, minNotional },
        '[VALIDATOR] 📊 Reglas SPOT'
      );
      ...
      if (truncatedQuantity < minQty) {
        ...
        const available =
          balances[action === 'buy' ? quoteAsset : baseAsset] || 0;
        if (available > 0) {
          ...
          errors.push(
            `❌ La cantidad (${truncatedQuantity})${baseAsset} es menor al mínimo permitido (${minQty})${baseAsset}. Necesitás al menos ${minUsdtRequired.toFixed(
              ...
              2
            )}% de tu balance disponible (${available.toFixed(
              2
            )} ${quoteAsset}).`
          );
        } else {
      ...
      if (remainder !== 0 && stepSize > 0) {
        errors.push(
          `❌ La cantidad '${truncatedQuantity}'${baseAsset} no es múltiplo válido de ${stepSize}`
        );
      }
      ...
      if (minNotional > 0 && notional < minNotional) {
        errors.push(
          `❌ Valor total de la orden (${notional.toFixed(
            2
          )}) menor al mínimo requerido (${minNotional.toFixed(2)})`
        );
      }
      ...
      if (errors.length > 0) {
        ...
        logger.warn(
          { errors: errorMsg },
          '[VALIDATOR] Errores de validación SPOT'
        );
      ...
      logger.error(
        { error: msg, originalError: error },
        '[VALIDATOR] Error validando SPOT'
      );
      return { valid: false, error: msg, log_summary: msg };
    }
  }
  ...
  private async _validateFutures(
    ...
    try {
      ...
      const stepSize = parseFloat(filters['LOT_SIZE']?.stepSize || '0');
      const minQty = parseFloat(filters['LOT_SIZE']?.minQty || '0');
      const maxQty = parseFloat(filters['LOT_SIZE']?.maxQty || '0');
      ...
      if (truncatedQuantity < minQty) {
        errors.push(
          `❌ La cantidad (${truncatedQuantity}) es menor al mínimo permitido (${minQty})`
        );
      }
      if (truncatedQuantity > maxQty) {
        errors.push(
          `❌ La cantidad (${truncatedQuantity}) excede el máximo permitido (${maxQty})`
        );
      }
      ...
      if (close_position) {
        const closeErrors = await this._validateClosePosition(
          symbol,
          sideResult.positionSide,
          truncatedQuantity
        );
      ...
      if (errors.length > 0) {
        ...
        logger.warn(
          { errors: errorMsg },
          '[VALIDATOR] Errores de validación FUTURES'
        );
        return { valid: false, error: errorMsg, log_summary: errorMsg };
      }
      ...
      const msg = `❌ Error inesperado en la validación de FUTURES: ${
        error.message || error
      }`;
      logger.error(
        { error: msg, originalError: error },
        '[VALIDATOR] Error validando FUTURES'
      );
      return { valid: false, error: msg, log_summary: msg };
    }
  }
  ...
  private async _validateClosePosition(
    ...
    try {
      ...
      errors.push(
        `❌ Error al validar cierre de posición: ${error.message || error}`
      );
    }
    return errors;
  }
  ...
  private async _validateOpenPosition(
    ...
    if ('PRICE_FILTER' in filters) {
      const tickSize = parseFloat(filters['PRICE_FILTER'].tickSize || '0');
      const minPrice = parseFloat(filters['PRICE_FILTER'].minPrice || '0');
      const maxPrice = parseFloat(filters['PRICE_FILTER'].maxPrice || '0');

      const pricePrecision = Math.max(
        0,
        Math.round(-Math.log10(tickSize || 0.00000001))
      );
      const roundedPrice = parseFloat(price.toFixed(pricePrecision));

      if (
        (minPrice > 0 && roundedPrice < minPrice) ||
        (maxPrice > 0 && roundedPrice > maxPrice)
      ) {
        errors.push(
          `❌ El precio ${roundedPrice} está fuera de los límites permitidos (${minPrice} - ${maxPrice}).`
        );
      }
    }
    ...
    logger.info(
      { leverage, price, notional },
      '[VALIDATOR] 📊 FUTURES — leverage, precio, notional'
    );

    // Validar NOTIONAL mínimo
    const minNotional =
      parseFloat(filters['MIN_NOTIONAL']?.notional || '0') ||
      parseFloat(filters['MIN_NOTIONAL']?.minNotional || '0') ||
      parseFloat(filters['NOTIONAL']?.notional || '0') ||
      parseFloat(filters['NOTIONAL']?.minNotional || '0');
    ...
    if (minNotional > 0 && notional < minNotional) {
      errors.push(
        `❌ El valor total de la orden (${notional.toFixed(
          2
        )} ${quoteAsset}) es menor al mínimo permitido (${minNotional.toFixed(
          2
        )} ${quoteAsset}).`
      );
    }
